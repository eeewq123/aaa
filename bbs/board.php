...
if (isset($write['wr_is_comment']) && $write['wr_is_comment']) {
    goto_url(G5_URL.'/b/'.$bo_table.'/'.$write['wr_parent'].'#c_'.$wr_id);
}
...
if (isset($wr_id) && $wr_id) {
    ...
    if (isset($group['gr_use_access']) && $group['gr_use_access']) {
        if ($is_guest) {
            ...
            alert($msg, './login.php?wr_id='.$wr_id.$qstr.'&amp;url='.urlencode(G5_URL.'/b/'.$bo_table.'/'.$wr_id.$qstr));
        }
    ...
    if ($member['mb_level'] < $board['bo_read_level']) {
        if ($is_member)
            ...
            alert('글을 읽을 권한이 없습니다.\\n\\n회원이시라면 로그인 후 이용해 보십시오.', './login.php?wr_id='.$wr_id.$qstr.'&amp;url='.urlencode(G5_URL.'/b/'.$bo_table.'/'.$wr_id.$qstr));
    }
    ...
    if ($member['mb_level'] < $board['bo_list_level']) {
        if ($member['mb_id'])
            ...
            alert('목록을 볼 권한이 없습니다.\\n\\n회원이시라면 로그인 후 이용해 보십시오.', './login.php?'.$qstr.'&url='.urlencode(G5_URL.'/b/'.$bo_table.($qstr?'&amp;':'')));
    }
